...
const WeatherBar = () => {
  ...
  const setSelectedCity = useStore.getState().setSelectedCity;
  useEffect(() => {
    WeatherApi();
    return () => {
      cleanData();
  ...
  return (
    <>
      {weathers.length === 0 ? (
        ...
        <ul className="absolute left-0 w-52 h-full pt-10 bg-black/50">
          {weathers.map((city) => (
            <li
              key={city.name}
            ...
            >
              <div className="w-full flex justify-between">
                ...
                <div className="font-thin text-3xl text-light-gray">
                  {city.temp.slice(0, 2)}°
                </div>
              </div>
